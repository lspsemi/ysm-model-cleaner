 ...
 *
 */
void xchacha_hchacha20(uint8_t *out, const uint8_t *in, const uint8_t *k, const uint32_t rounds){
    int i;
...
void xchacha_keysetup(XChaCha_ctx *ctx, const uint8_t *k, const uint8_t *iv){
     ...
     * as normal.
     */
    xchacha_hchacha20(k2, iv, k, ctx->rounds);
...
void xchacha_encrypt_bytes(XChaCha_ctx *ctx, const uint8_t *m, uint8_t *c, uint32_t bytes){
    ...
    for (;;) {
        ...
        /* Do x rounds instead of 8 */
        for (i = ctx->rounds;i > 0;i -= 2) {
            QUARTERROUND( x0, x4, x8,x12)
            QUARTERROUND( x1, x5, x9,x13)
            QUARTERROUND( x2, x6,x10,x14)
            QUARTERROUND( x3, x7,x11,x15)
