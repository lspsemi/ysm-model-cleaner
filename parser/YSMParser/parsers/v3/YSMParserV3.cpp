...
std::vector<uint8_t> YSMParserV3::ParseModels(BufferReader& reader)
{
	...
	for (uint32_t i = 0; i < sizeOfBones; i++)
	{
		...
		// 0.3.5 treated this field as a hard error when non-zero. 0.3.6 keeps
		// parsing, but records it so GUI tools can detect stripped anti-parser data.
		const size_t antiParserGuardOffset = reader.offset;
		const uint64_t antiParserGuardValue = reader.readVarint();
		if (antiParserGuardValue != 0) {
			recordDiagnostic(
				"anti_parser",
				"v3_model_bone_unknown_guard_nonzero",
				"Non-zero V3 model bone guard field skipped by YSMParser 0.3.6.",
				"model_bone:" + bone.name,
				static_cast<uint64_t>(antiParserGuardOffset),
				antiParserGuardValue
			);
		}
...
void YSMParserV3::saveToDirectory(std::string output_directory)
{
	...
	if (!getDiagnostics().empty()) {
		...
		diagnostics["tool"] = "YSMParser";
		diagnostics["version"] = "0.3.6-diagnostics";
		diagnostics["event_count"] = getDiagnostics().size();
		diagnostics["events"] = nlohmann::ordered_json::array();

		for (const auto& item : getDiagnostics()) {
			nlohmann::ordered_json event = nlohmann::ordered_json::object();
			...
			event["offset_hex"] = "0x" + [&]() {
				...
				oss << std::uppercase << std::hex << item.offset;
				return oss.str();
			}();
		...
		const std::string text = diagnostics.dump(2);
		saveFile(dirPath / "_ysm_parser_diagnostics.json", std::vector<uint8_t>(text.begin(), text.end()));
	}
}
